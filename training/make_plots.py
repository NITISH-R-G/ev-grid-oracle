...
def plot_episode_trajectories(
    rows: list[dict[str, Any]], data: dict[str, Any], out_dir: Path
) -> Path | None:
    if not rows:
        return None
    ...
    axes[0].set_title(
        "Per-episode trajectories (paired worlds — lines should diverge when LoRA is active)"
    )
...
def plot_delta_histograms(rows: list[dict[str, Any]], out_dir: Path) -> Path | None:
    if not rows:
        return None
    d_wait = [
        float(r["oracle"]["avg_wait"]) - float(r["baseline"]["avg_wait"]) for r in rows
    ]
    d_peak = [
        float(r["oracle"]["peak_violations"]) - float(r["baseline"]["peak_violations"])
        for r in rows
    ]
    d_stress = [
        float(r["oracle"]["grid_stress_events"])
        - float(r["baseline"]["grid_stress_events"])
        for r in rows
    ]
    ...
    fig.suptitle(
        "Paired deltas per episode — mass left of 0 means oracle is better (lower wait / fewer violations)"
    )
    fig.tight_layout()
...
def plot_reward_breakdown(data: dict[str, Any], out_dir: Path) -> Path | None:
    ...
    ax.set_xlabel(
        "Mean per-step reward component (episode average, then mean over episodes)"
    )
...
def plot_boxplots(rows: list[dict[str, Any]], out_dir: Path) -> Path | None:
    ...
    fig.suptitle(
        "Distribution over episodes — spread shows world variance; separation shows learning"
    )
    fig.tight_layout()
...
def plot_oracle_win_rates(rows: list[dict[str, Any]], out_dir: Path) -> Path | None:
    ...
    items = [
        ...
        (
            "Oracle fewer stress ticks",
            rate(True, "grid_stress_events", "grid_stress_events"),
        ),
        ("Oracle higher renewable", rate(False, "renewable_mean", "renewable_mean")),
        (
            "Oracle fewer anti-cheat steps",
...
def plot_binary_timeline(rows: list[dict[str, Any]], out_dir: Path) -> Path | None:
    ...
    im = ax.imshow(
        mat, aspect="auto", cmap="YlOrRd", vmin=0, vmax=1, interpolation="nearest"
    )
    ax.set_yticks(range(len(keys)))
    ax.set_yticklabels([k[0] for k in keys])
    ax.set_xlabel("Episode index")
    ax.set_title(
        "Baseline binary stress flags over paired episodes (dark = event occurred)"
    )
...
def plot_mcnemar_summary(fair_path: Path, out_dir: Path) -> Path | None:
    ...
    ax.set_title(
        "Paired significance — low p means baseline vs oracle differ on binary outcomes"
    )
    ax.legend()
    ax.set_xlim(0, 1)
    ax.grid(axis="x", alpha=0.25)
...
def plot_dashboard_grid(
    ...
    wins = sum(
        1
        for r in rows
        if float(r["oracle"]["avg_wait"]) < float(r["baseline"]["avg_wait"]) - 1e-9
    )
    ax5.bar(
        ["Oracle wins\n(lower wait)"],
        [100 * wins / len(rows)],
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Generate KPI + paired-eval visualization suite for README / judges."
    )
