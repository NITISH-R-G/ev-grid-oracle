...
def _graph_route_km(
    city_graph: nx.Graph, lat1: float, lng1: float, lat2: float, lng2: float
) -> float | None:
    """
    ...
    try:
        n1 = min(
            city_graph.nodes,
            key=lambda n: _haversine_km(lat1, lng1, float(n[0]), float(n[1])),
        )
        n2 = min(
            city_graph.nodes,
            key=lambda n: _haversine_km(lat2, lng2, float(n[0]), float(n[1])),
        )
...
def compute_reward(
    *,
    ...
    avg_wait = (
        mean([s.avg_wait_minutes for s in next_state.stations])
        if next_state.stations
        else 0.0
    )
    ...
    overloaded = sum(
        1
        for s in next_state.stations
        if (s.occupied_slots / max(1, s.total_slots)) > 0.85
    )
    ...
    if action.action_type == ActionType.route:
        station = next(
            (s for s in next_state.stations if s.station_id == action.station_id), None
        )
        # mypy complains about variable type "EVRequest" being reassigned to EVRequest|None
        ev_req = next(
            (e for e in prev_state.pending_evs if e.ev_id == action.ev_id), None
        )
        src = next(
            (
                s
                for s in prev_state.stations
                if s.neighborhood_slug == (ev_req.neighborhood_slug if ev_req else "")
            ),
            None,
        )
        ...
        if station is None:
            ...
            add_flag(
                "phantom_capacity",
                "Could not resolve EV neighborhood to a station anchor",
            )
            shaping_ok = False
        else:
            # Feasibility: SOC must be able to reach station (very rough range model)
            dist_km = _graph_route_km(
                city_graph, src.lat, src.lng, station.lat, station.lng
            )
            if dist_km is None:
            ...
            if station.occupied_slots >= station.total_slots:
                ...
                add_flag(
                    "phantom_capacity",
                    f"Station full: {station.station_id} occupied={station.occupied_slots}/{station.total_slots}",
                )
                shaping_ok = False
            if station.queue_length > 5:
                ...
                add_flag(
                    "queue_piling",
                    f"Queue too long at {station.station_id}: {station.queue_length}",
                )
                shaping_ok = False
            ...
            if float(next_state.grid_load_pct) >= 0.92:
                ...
                add_flag(
                    ...
                    f"Grid load critical: {next_state.grid_load_pct * 100:.1f}%",
                )
                shaping_ok = False
    ...
    if action.action_type == ActionType.defer:
        ...
        if ev_req2 is not None and int(action.defer_minutes) > int(
            ev_req2.max_wait_minutes
        ):
...
def split_role_rewards(
    rb: dict[str, float], *, grid_directive_ok: bool, has_meaningful_messages: bool
