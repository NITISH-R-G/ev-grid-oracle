...
STATIONS: list[StationSpec] = [
    StationSpec(
        "BLR-01", "koramangala", "Koramangala", 12.9352, 77.6245, ChargerType.fast, 12
    ),
    StationSpec(
        "BLR-02",
        "whitefield",
    ...
    StationSpec(
        "BLR-03", "hsr_layout", "HSR Layout", 12.9116, 77.6389, ChargerType.fast, 10
    ),
    StationSpec(
        "BLR-04", "indiranagar", "Indiranagar", 12.9784, 77.6408, ChargerType.fast, 12
    ),
    StationSpec(
        "BLR-05",
        "electronic_city",
    ...
    StationSpec(
        "BLR-06", "marathahalli", "Marathahalli", 12.9591, 77.7006, ChargerType.fast, 12
    ),
    StationSpec(
        "BLR-07", "jayanagar", "Jayanagar", 12.9308, 77.5832, ChargerType.slow, 10
    ),
    StationSpec(
        "BLR-08", "yeshwanthpur", "Yeshwanthpur", 13.0245, 77.5497, ChargerType.fast, 12
    ),
    StationSpec(
        "BLR-09", "hebbal", "Hebbal", 13.0350, 77.5970, ChargerType.ultra_fast, 12
    ),
    StationSpec(
        "BLR-10", "sarjapur", "Sarjapur", 12.9010, 77.6960, ChargerType.slow, 8
    ),
    StationSpec(
        "BLR-11", "mg_road", "MG Road", 12.9757, 77.6070, ChargerType.ultra_fast, 12
    ),
    StationSpec(
        "BLR-12", "bellandur", "Bellandur", 12.9258, 77.6681, ChargerType.fast, 12
    ),
    StationSpec(
        "BLR-13", "bannerghatta", "Bannerghatta", 12.8636, 77.5771, ChargerType.slow, 10
    ),
    StationSpec(
        "BLR-14", "rajajinagar", "Rajajinagar", 12.9890, 77.5522, ChargerType.fast, 12
    ),
    StationSpec(
        "BLR-15", "jp_nagar", "JP Nagar", 12.9063, 77.5857, ChargerType.slow, 10
    ),
    StationSpec(
        "BLR-16", "btm_layout", "BTM Layout", 12.9165, 77.6101, ChargerType.fast, 10
    ),
    StationSpec(
        "BLR-17",
        "cunningham_road",
    ...
    StationSpec(
        "BLR-18", "yelahanka", "Yelahanka", 13.1004, 77.5963, ChargerType.slow, 10
    ),
    StationSpec("BLR-19", "kengeri", "Kengeri", 12.9140, 77.4830, ChargerType.slow, 8),
    StationSpec(
        "BLR-20", "tumkur_road", "Tumkur Road", 13.0250, 77.5100, ChargerType.slow, 12
    ),
    StationSpec(
        "BLR-21",
        "old_airport_road",
    ...
    StationSpec(
        "BLR-22", "kr_puram", "KR Puram", 13.0050, 77.6950, ChargerType.fast, 10
    ),
    StationSpec(
        "BLR-23", "silk_board", "Silk Board", 12.9176, 77.6228, ChargerType.fast, 12
    ),
    StationSpec(
        "BLR-24",
        "cv_raman_nagar",
...
def _edge_minutes(
    a: StationSpec, b: StationSpec, *, minutes_per_km: float = 5.0, base: float = 2.0
) -> float:
    ...
    try:
        return float(
            nx.shortest_path_length(
                g, from_station_id, to_station_id, weight="weight_minutes"
            )
        )
