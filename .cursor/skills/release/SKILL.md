...
1. **Stage envs with `scripts/prepare_hf_deployment.sh`.** Default arguments deploy every _deployable_ env from `envs/`. Pass `--env <name>` to target a subset. The script:
